    ...
    <ul style="margin-left:0" class="nav navbar-nav pull-left">
        ...
        <li class="dropdown hidden-xs active">
           <?php $page =($active_page == 'settings' && $active_action == 'item_group') ? 'active tab' : '' ?>
            <?php echo $this->Html->link("<span class='count'>Item Group</span>", array('controller' => 'settings', 'action' => 'item_group' ,'purchasing?'.rand(1000,9999).'='.date("is"),'plugin' => false),array('escape' => false,'class' => 'btn')); ?>
